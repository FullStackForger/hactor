...
var initializing = false, fnTest = /xyz/.test(function(){xyz;}) ? /\b_super\b/ : /.*/, Class;
...
Class.extend = function(properties, staticProperties) {
	...
	if (properties !== undefined) {
		if (staticProperties.constructor !== Object) throw new Error('\'properties\' must be an object');
	...
	if (staticProperties !== undefined) {
		if (staticProperties.constructor !== Object) throw new Error('staticProperties must be an object');
		
		for (var staticName in staticProperties) {
			if (staticProperties.hasOwnProperty(staticName)) {
				Class[staticName] = staticProperties[key];
			}
		}
	}
